# 🌌 Descartes

A sleek, responsive, and glassmorphic personal food tracking application built with **React**, **Vite**, and **Supabase**. Named after René Descartes, this app helps you log and reflect on your daily meals with simplicity and elegance.

---

## ✨ Features

- **🔐 Secure Authentication:** Ready-to-use login and signup interface powered by Supabase Auth (supports email/password and social login providers).
- **🍽️ Intuitive Meal Logging:** Log what you ate with precise time pickers, a date selector, and custom categories.
- **🏷️ Smart Meal Labels:** Classify your food entries under custom labels: *Breakfast*, *Lunch*, *Dinner*, and *Snack*.
- **📊 Chronological History:** Review all your previous meal entries structured clearly in an elegant history stream.
- **🎨 Glassmorphic UI/UX:** Built with modern CSS variables, responsive design, smooth hover transitions, and dark-mode aesthetics.

---

## 🛠️ Tech Stack

- **Frontend:** [React](https://react.dev/) + [Vite](https://vite.dev/)
- **Styling:** Vanilla CSS (Modern CSS Variables & Glassmorphism)
- **Backend-as-a-Service:** [Supabase](https://supabase.com/) (Database, Auth, and Auto-APIs)
- **Icons:** [Lucide React](https://lucide.dev/)
- **Routing:** [React Router DOM v7](https://reactrouter.com/)

---

## 🚀 Getting Started

### 1. Prerequisites

Make sure you have Node.js (version 18+ recommended) installed on your local machine.

### 2. Installation

Clone the repository and install the dependencies:

```bash
git clone https://github.com/thechaos16/Descartes.git
cd Descartes
npm install
```

### 3. Environment Setup

Create a `.env` file in the root directory of the project and add your Supabase credentials:

```env
VITE_SUPABASE_URL=your_supabase_project_url
VITE_SUPABASE_ANON_KEY=your_supabase_anonymous_key
```

### 4. Supabase Database Schema

To store your food entries and tasks, create tables named `food_entries` and `todos` in your Supabase database. You can execute the following SQL in your Supabase SQL Editor:

```sql
-- Food entries table
create table public.food_entries (
  id bigint generated by default as identity primary key,
  created_at timestamp with time zone default timezone('utc'::text, now()) not null,
  date date not null,
  time_str text not null,
  food text not null,
  labels text[] default '{}'::text[] not null,
  user_id uuid default auth.uid() not null
);

-- Enable RLS for food_entries
alter table public.food_entries enable row level security;

-- Policies for food_entries
create policy "Users can view their own entries"
  on public.food_entries for select
  using (auth.uid() = user_id);

create policy "Users can insert their own entries"
  on public.food_entries for insert
  with check (auth.uid() = user_id);


-- Todos table
create table public.todos (
  id bigint generated by default as identity primary key,
  created_at timestamp with time zone default timezone('utc'::text, now()) not null,
  date date not null,
  title text not null,
  status text default 'pending'::text not null, -- pending, completed, cancelled
  user_id uuid default auth.uid() not null
);

-- Enable RLS for todos
alter table public.todos enable row level security;

-- Policies for todos
create policy "Users can view their own todos"
  on public.todos for select
  using (auth.uid() = user_id);

create policy "Users can insert their own todos"
  on public.todos for insert
  with check (auth.uid() = user_id);

create policy "Users can update their own todos"
  on public.todos for update
  using (auth.uid() = user_id);

create policy "Users can delete their own todos"
  on public.todos for delete
  using (auth.uid() = user_id);


-- Bookmarks table
create table public.bookmarks (
  id bigint generated by default as identity primary key,
  created_at timestamp with time zone default timezone('utc'::text, now()) not null,
  url text not null,
  normalized_url text not null,
  title text not null,
  is_read boolean default false not null,
  user_id uuid default auth.uid() not null,
  
  -- Prevent duplicate bookmarks for the same user
  constraint unique_user_bookmark unique (user_id, normalized_url)
);

-- Enable RLS for bookmarks
alter table public.bookmarks enable row level security;

-- Policies for bookmarks
create policy "Users can view their own bookmarks"
  on public.bookmarks for select
  using (auth.uid() = user_id);

create policy "Users can insert their own bookmarks"
  on public.bookmarks for insert
  with check (auth.uid() = user_id);

create policy "Users can update their own bookmarks"
  on public.bookmarks for update
  using (auth.uid() = user_id);

create policy "Users can delete their own bookmarks"
  on public.bookmarks for delete
  using (auth.uid() = user_id);


-- Happiness table
create table public.happiness (
  id bigint generated by default as identity primary key,
  created_at timestamp with time zone default timezone('utc'::text, now()) not null,
  date date not null,
  time_str text not null,
  moment text not null,
  category text not null,
  user_id uuid default auth.uid() not null
);

-- Enable RLS for happiness
alter table public.happiness enable row level security;

-- Policies for happiness
create policy "Users can view their own happiness entries"
  on public.happiness for select
  using (auth.uid() = user_id);

create policy "Users can insert their own happiness entries"
  on public.happiness for insert
  with check (auth.uid() = user_id);

create policy "Users can update their own happiness entries"
  on public.happiness for update
  using (auth.uid() = user_id);

create policy "Users can delete their own happiness entries"
  on public.happiness for delete
  using (auth.uid() = user_id);
```



### 5. Running Locally

Start the development server:

```bash
npm run dev
```

Open [http://localhost:5173](http://localhost:5173) in your browser to view the application.

### 6. Building for Production

To create a production-ready optimized build:

```bash
npm run build
npm run preview
```

### 7. Production Deployment & Security

When deploying your Descartes application to production (e.g., to Vercel, Netlify, or AWS), follow these guidelines to handle environment variables and keep your keys secure.

#### A. Environment Variables in Deployment
Do **not** commit credentials (like `.env` or `.env.production`) to source control. Instead, set them in your hosting provider's dashboard:
- **Vercel:** Project Settings > Environment Variables
- **Netlify:** Site configuration > Environment variables
- Add both `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY` as production environment variables.

#### B. Anon Key Security (Is it safe to expose?)
Yes! In a frontend-only Single Page Application (SPA), Vite embeds environment variables prefixed with `VITE_` directly into the compiled JavaScript bundle. This means anyone inspecting the page can see your `VITE_SUPABASE_ANON_KEY`.

**How security is maintained:**
- The **Anon Key** is a public key. It is designed to be client-facing.
- Security in Supabase is enforced on the database level via **Row Level Security (RLS)**, not by hiding the key.
- As long as you enable RLS on your tables (as shown in Step 4) and define policies (e.g., restricting access to `auth.uid() = user_id`), users cannot access other users' data, even if they have the anon key.

#### C. How to Completely Hide the Anon Key (Optional)
If your application requirements strictly demand that no keys are visible in the browser, you cannot connect to Supabase directly from the React client. You must implement a **Backend Proxy Server**:
1. Create a server (e.g., Node.js/Express, Next.js API route, Serverless Function, or Cloudflare Worker).
2. Store the Supabase URL and Anon Key (or Service Role Key) only in the backend server's environment variables.
3. Update your React application to make HTTP requests to your backend server instead of using the Supabase client directly.
4. Your backend server receives the requests, verifies the user session (usually via JWT), executes queries to Supabase, and returns the data to the frontend.

*Note: Using a proxy server adds extra latency, complexity, and maintenance overhead, and disables Supabase's out-of-the-box features like real-time subscriptions.*

---

## 📂 Project Structure

```text
descartes/
├── src/
│   ├── components/
│   │   ├── Auth.jsx          # Login, Sign Up, & Password Recovery UI
│   │   ├── FoodTracker.jsx   # Form interface to log new meals
│   │   └── HistoryPage.jsx   # List/Timeline view of past entries
│   ├── lib/
│   │   └── supabaseClient.js # Supabase client initialization
│   ├── App.jsx               # Main application routing and core logic
│   ├── index.css             # Main styling system, design tokens, & layout
│   └── main.jsx              # React mounting entrypoint
├── public/                   # Static assets
└── package.json              # Dependencies and scripts
```
